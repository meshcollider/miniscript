...
template<typename Key>
void BuildBack(NodeType nt, std::vector<NodeRef<Key>>& constructed, bool reverse = false)
{
    ...
    constructed.pop_back();
    if (reverse) {
        constructed.push_back(MakeNodeRef<Key>(nt, Vector(std::move(right), std::move(left))));
    } else {
